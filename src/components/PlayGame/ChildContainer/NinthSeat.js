...
export default class NinthSeat extends Component {
    ...
    componentDidMount() {
        ...
        axios.post(API_URL + '/room/get-data', { roomName: this.props.roomName })
            .then(async (res) => {
                ...
                if (res.data.ninthPlayer && this._isMounted) {
                    let ninthPlayer = res.data.ninthPlayer;
                    this.updateState(ninthPlayer);
                }
            })
            .catch(err => console.log(err));

        socketRoom.on('game_room_update', (data) => {
            if (data.ninthPlayer && this._isMounted) {
                let ninthPlayer = data.ninthPlayer;
                this.updateState(ninthPlayer);
            }
        })
        ...
        socketRoom.on('update_entire_room', (data) => {
            if (this._isMounted) {
                this.updateState(data.ninthPlayer);
            }
        })
    ...
    render() {
        return (
            <View>
                <ImageBackground
                    style={[styles.imageChildTable, { marginHorizontal: 5, borderWidth: this.props.position == 9 ? 1 : 0 }]}
                    ...
                    {this.state.userName != null && (
                        <View style={{ flex: 1, flexDirection: 'row' }}>
                            ...
                            >
                                ...
                                <TouchableOpacity
                                    activeOpacity={0.8}
                                    style={{ height: 0.55 * this.state.widthAvatarContainer, width: 0.55 * this.state.widthAvatarContainer, marginRight: 10, borderRadius: 5, marginBottom: 1 }}
                                    onPress={() => {
                                ...
                                </View>
                                {
                                    this.state.cardFirst != null && this.state.cardSecond != null && this.state.cardThird != null && (
                                        <View style={styles.containerCard}>
                                            <TouchableOpacity
                                                activeOpacity={this.props.position == 9 ? 0.8 : 1}
                                                style={{ flex: 1, alignItems: 'center' }}
                                                onPress={() => {
                                                    if (this.props.position == 9 && !this.state.flipCardFirst) {
                                            ...
                                            <TouchableOpacity
                                                ...
                                                onPress={() => {
                                                    if (this.props.position == 9 && !this.state.flipCardSecond) {
                                            ...
                                            <TouchableOpacity
                                                ...
                                                style={{ flex: 1 }}
                                                onPress={() => {
                                                    if (this.props.position == 9 && !this.state.flipCardThird) {
