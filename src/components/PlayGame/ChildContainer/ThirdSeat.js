...
export default class ThirdSeat extends Component {
    ...
    componentDidMount() {
        ...
        axios.post(API_URL + '/room/get-data', { roomName: this.props.roomName })
            .then(async (res) => {
                ...
                if (res.data.thirdPlayer && this._isMounted) {
                    let thirdPlayer = res.data.thirdPlayer;
                    this.updateState(thirdPlayer);
                }
            })
            .catch(err => console.log(err));

        socketRoom.on('game_room_update', (data) => {
            if (data.thirdPlayer && this._isMounted) {
                let thirdPlayer = data.thirdPlayer;
                this.updateState(thirdPlayer);
            }
        })
        ...
        socketRoom.on('update_entire_room', (data) => {
            if (this._isMounted) {
                this.updateState(data.thirdPlayer);
            }
        })
    ...
    render() {
        return (
            <View>
                <ImageBackground
                    style={[styles.imageChildTable, { marginHorizontal: 5, borderWidth: this.props.position == 3 ? 1 : 0 }]}
                    ...
                    {this.state.userName != null && (
                        <View style={{ flex: 1, flexDirection: 'row' }}>
                            ...
                            >
                                ...
                                <TouchableOpacity
                                    activeOpacity={0.8}
                                    style={{ height: 0.55 * this.state.widthAvatarContainer, width: 0.55 * this.state.widthAvatarContainer, marginRight: 10, borderRadius: 5, marginTop: 2 }}
                                    onPress={() => {
                                ...
                                </View>
                                {
                                    this.state.cardFirst != null && this.state.cardSecond != null && this.state.cardThird != null && (
                                        <View style={styles.containerCard}>
                                            <TouchableOpacity
                                                activeOpacity={this.props.position == 3 ? 0.8 : 1}
                                                style={{ flex: 1, alignItems: 'center' }}
                                                onPress={() => {
                                                    if (this.props.position == 3 && !this.state.flipCardFirst) {
                                            ...
                                            <TouchableOpacity
                                                ...
                                                onPress={() => {
                                                    if (this.props.position == 3 && !this.state.flipCardSecond) {
                                            ...
                                            <TouchableOpacity
                                                ...
                                                style={{ flex: 1 }}
                                                onPress={() => {
                                                    if (this.props.position == 3 && !this.state.flipCardThird) {
