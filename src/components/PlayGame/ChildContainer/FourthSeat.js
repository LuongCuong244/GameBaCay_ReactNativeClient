...
export default class FourthSeat extends Component {
    ...
    componentDidMount() {
        ...
        axios.post(API_URL + '/room/get-data', { roomName: this.props.roomName })
            .then(async (res) => {
                ...
                if (res.data.fourthPlayer && this._isMounted) {
                    let fourthPlayer = res.data.fourthPlayer;
                    this.updateState(fourthPlayer);
                }
            })
            .catch(err => console.log(err));

        socketRoom.on('game_room_update', (data) => {
            if (data.fourthPlayer && this._isMounted) {
                let fourthPlayer = data.fourthPlayer;
                this.updateState(fourthPlayer);
            }
        })
        ...
        socketRoom.on('update_entire_room', (data) => {
            if (this._isMounted) {
                this.updateState(data.fourthPlayer);
            }
        })
    ...
    render() {
        return (
            <View>
                <ImageBackground
                    style={[styles.imageChildTable, { marginHorizontal: 5, borderWidth: this.props.position == 4 ? 1 : 0 }]}
                    ...
                    {this.state.userName != null && (
                        <View style={{ flex: 1, flexDirection: 'row' }}>
                            ...
                            >
                                ...
                                <TouchableOpacity
                                    activeOpacity={0.8}
                                    style={{ height: 0.55 * this.state.widthAvatarContainer, width: 0.55 * this.state.widthAvatarContainer, marginRight: '7%', borderRadius: 5 }}
                                    onPress={() => {
                                ...
                                </View>
                                {
                                    this.state.cardFirst != null && this.state.cardSecond != null && this.state.cardThird != null && (
                                        <View style={styles.containerCard}>
                                            <TouchableOpacity
                                                activeOpacity={this.props.position == 4 ? 0.8 : 1}
                                                style={{ flex: 1, alignItems: 'center' }}
                                                onPress={() => {
                                                    if (this.props.position == 4 && !this.state.flipCardFirst) {
                                            ...
                                            <TouchableOpacity
                                                ...
                                                onPress={() => {
                                                    if (this.props.position == 4 && !this.state.flipCardSecond) {
                                            ...
                                            <TouchableOpacity
                                                ...
                                                style={{ flex: 1 }}
                                                onPress={() => {
                                                    if (this.props.position == 4 && !this.state.flipCardThird) {
