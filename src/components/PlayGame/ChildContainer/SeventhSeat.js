...
export default class SeventhSeat extends Component {
    ...
    componentDidMount() {
        ...
        axios.post(API_URL + '/room/get-data', { roomName: this.props.roomName })
            .then(async (res) => {
                ...
                if (res.data.seventhPlayer && this._isMounted) {
                    let seventhPlayer = res.data.seventhPlayer;
                    this.updateState(seventhPlayer);
                }
            })
            .catch(err => console.log(err));

        socketRoom.on('game_room_update', (data) => {
            if (data.seventhPlayer && this._isMounted) {
                let seventhPlayer = data.seventhPlayer;
                this.updateState(seventhPlayer);
            }
        })
        ...
        socketRoom.on('update_entire_room', (data) => {
            if (this._isMounted) {
                this.updateState(data.seventhPlayer);
            }
        })
    ...
    render() {
        return (
            <View>
                <ImageBackground
                    style={[styles.imageChildTable, { marginHorizontal: 5, borderWidth: this.props.position == 7 ? 1 : 0 }]}
                                ...
                                </View>
                                {
                                    this.state.cardFirst != null && this.state.cardSecond != null && this.state.cardThird != null && (
                                        <View style={styles.containerCard}>
                                            <TouchableOpacity
                                                activeOpacity={this.props.position == 7 ? 0.8 : 1}
                                                style={{ flex: 1, alignItems: 'center' }}
                                                onPress={() => {
                                                    if (this.props.position == 7 && !this.state.flipCardFirst) {
                                            ...
                                            <TouchableOpacity
                                                ...
                                                onPress={() => {
                                                    if (this.props.position == 7 && !this.state.flipCardSecond) {
                                            ...
                                            <TouchableOpacity
                                                ...
                                                style={{ flex: 1 }}
                                                onPress={() => {
                                                    if (this.props.position == 7 && !this.state.flipCardThird) {
